...
namespace mu2e
{
  class CrvDQMcollector : public art::EDAnalyzer
  {
    ...
    struct Config
    {
      ...
      //which histograms get per-subrun / last-N-events copies; one block shared
      //by all three helpers. Omit for job-only, which is what hadd wants.
      fhicl::OptionalDelegatedParameter segmentation{Name("segmentation"), Comment("histogram segmentation rules; see Offline/DQMHelpers/README.md")};
    };
